...
function checkEmailAndPassword(email, pass, callback) {
    var sqlCheckEmailAndPassword = 'select id, email from passwords where password = ? and starttime < ? and (expiretime is null or expiretime > ?)';
    var currentTime = stringUtil.getUTCNow();
    ...
    dbPool.runQueryWithParams(sqlCheckEmailAndPassword, [pass, currentTime, currentTime], function(err, result){
        ...
        if(result==null || result.length==0) {
            var errorMsg ="can't find password for email: " + email;
    ...
    checkEmailAndPassword(email, pass, function(err, result){
        ...
        getUnfinishedSession(email, function(err, unfinishedSession){
            ...
            if(unfinishedSession!=null) {
                ...
                returnObj.session = new Session(result.id, email, unfinishedSession.testId);
                ...
                returnObj.session = new Session(result.id, email, testId);
